...
public class TabFileReader {
    ...
    public static void readTextFile(String fileName, char separator, String packagePath) {
        words = new String[1000][100];
        String line;
        String fullfilename;
        if (packagePath == null) fullfilename = fileName;
        else fullfilename = packagePath + "/" + fileName;
        int nb = 0;
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                for (int j = 0; j < fields.length; j++) words[nb][j] = fields[j].trim();
                nb++;
            }
            nrow = nb;
            in.close();
